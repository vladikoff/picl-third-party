...
function addControlPage(worker) {
    ...
    worker.port.on("from-content", function (data) {
        ...
        if (data.name == "storeCredentials") {
            ss.storage.credentials = data.credentials;
        }

        if (data.name == "loadCredentials") {
            worker.port.emit("to-content", {name: 'loadCredentials', data: { credentials: ss.storage.credentials }});
        }
    });
