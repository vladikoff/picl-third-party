...
(function() {
  ...
  } else if (((typeof window !== 'undefined' || typeof self !== 'undefined') && (typeof navigator !== 'undefined' && typeof navigator.userAgent === 'string')) || typeof Buffer !== "function") {
  ...
  Dropbox.Client = (function() {
    ...
    Client.prototype.readFile = function(path, options, callback) {
      ...
      if (typeof Buffer === "function" || typeof window !== 'undefined') {
  ...
  Dropbox.Drivers.Firefox = (function(_super) {
    ...
    Firefox.prototype.forgetCredentials = function(callback) {
      if (this.ss.storage.credentials && this.ss.storage.credentials[this.storageKey]) {
        this.ss.storage.credentials[this.storageKey] = null;
      }
      return callback();
    };

    Firefox.prototype.loadCredentials = function(callback) {
      if (this.ss.storage.credentials != null) {
        return callback(this.ss.storage.credentials[this.storageKey]);
      } else {
        return callback(null);
      }
    };

    Firefox.prototype.storeCredentials = function(credentials, callback) {
      if (this.ss.storage.credentials == null) {
        this.ss.storage.credentials = {};
      }
      this.ss.storage.credentials[this.storageKey] = credentials;
      return callback();
    };

    Firefox.prototype.doAuthorize = function(authUrl, token, tokenSecret, callback) {
      var data, onReady, tabs;
      ...
      return tabs.open({
        ...
        onReady: onReady = function(tab) {
          ...
          if (tab.title.indexOf('about:blank#?') === 0) {
            path = "oauth_receiver.html#?" + tab.title.split("#?")[1];
            Dropbox.Drivers.Firefox.oauthReceiver(path);
            tab.close();
            return callback();
          }
        }
  ...
  Dropbox.Drivers.NodeServer = (function() {
    function NodeServer(options) {
      ...
      mopen = 'open';
      this.fs = require(mfs);
      this.http = require(mhttp);
      this.open = require(mopen);
      this.callbacks = {};
  ...
  */
  ...
  if (typeof XMLHttpRequest !== 'undefined' && (typeof window !== 'undefined' || typeof self !== 'undefined') && typeof navigator !== 'undefined' && typeof navigator.userAgent === 'string') {
    ...
    if (typeof Buffer !== "function") {
      ...
      mxhr = 'xhr2';
      DropboxXhrRequest = require(mxhr);
    }
    DropboxXhrIeMode = false;
    DropboxXhrCanSendForms = false;
