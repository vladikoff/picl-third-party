...
(function() {
  ...
  Dropbox.Drivers.Cordova = (function(_super) {
   ...
   /* FIREFOX BROWSER */
    ...
    Dropbox.Drivers.FirefoxAddon = (function(_super) {
        __extends(FirefoxAddon, _super);

        function FirefoxAddon(options) {
            var _ref;

            options.receiverUrl = "about:blank";
            FirefoxAddon.__super__.constructor.call(this, options);
            this.rememberUser = true;
            _ref = this.computeUrl(options.receiverUrl), this.receiverUrl1 = _ref[0], this.receiverUrl2 = _ref[1];
        }

        FirefoxAddon.prototype.onAuthStateChange = function(client, callback) {
            ...
            superCall = (function() {
                return function() {
                    return FirefoxAddon.__super__.onAuthStateChange.call(_this, client, callback);
                };
            })();
            this.setStorageKey(client);


            console.log(client.authState);
            switch (client.authState) {
                case Dropbox.Client.RESET:
                    return this.loadCredentials(function(credentials) {
                        if (credentials) {
        ...
        FirefoxAddon.prototype.forgetCredentials = function(callback) {
            require("sdk/simple-storage").storage.credentials = null;
        };

        FirefoxAddon.prototype.loadCredentials = function(callback) {
            console.log('loadCredentials');
            var creds = require("sdk/simple-storage").storage.credentials;
            return callback(creds || null);;
        };

        FirefoxAddon.prototype.storeCredentials = function(credentials, callback) {
            console.log('storeCredentials');
            console.log(JSON.stringify(credentials));
            require("sdk/simple-storage").storage.credentials = credentials;
            callback();
            return this;
        };

        FirefoxAddon.prototype.doAuthorize = function(authUrl, token, tokenSecret, callback) {
            ...
            tabs.open({
                ...
                onReady: function onReady(tab) {
                    ...
                    if (tab.title.indexOf('about:blank#?') == 0) {
                        ...
                        Dropbox.Drivers.FirefoxAddon.oauthReceiver(path);
        ...
        FirefoxAddon.prototype.url = function(token) {
            console.log('url');
            return this.receiverUrl1 + encodeURIComponent(token) + this.receiverUrl2;
        };

        FirefoxAddon.prototype.listenForMessage = function(token, callback) {
            ...
            listener = function(event) {
                ...
                if (_this.locationToken(data) === token) {
                    token = null;
                    window.removeEventListener('message', listener);
                    Dropbox.Drivers.FirefoxAddon.onMessage.removeListener(listener);
                    return callback();
                }
            };
            ...
            return Dropbox.Drivers.FirefoxAddon.onMessage.addListener(listener);
        };

        FirefoxAddon.oauthReceiver = function(path) {
            console.log('oauthReceiver');
            return this.onMessage.dispatch(path);
        };

        FirefoxAddon.onMessage = new Dropbox.EventSource;
